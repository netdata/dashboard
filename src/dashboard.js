...
// todo remove when all vendors will be in the bundle
...
NETDATA.registry = {
    ...
    init: function () {
        ...
        NETDATA.registry.hello(NETDATA.serverDefault, function (data) {
            if (data) {
                NETDATA.registry.server = (data.registry || "").replace("https:", `${location.protocol}`);
...
NETDATA.loadRequiredJs = function (index, callback) {
    ...
    $.ajax({
        ...
        cache: true,
        // turning off because CRA loads it as XHR
        // todo check if it's needed by Agent Server
        // dataType: "script",
        xhrFields: {withCredentials: true} // required for the cookie
    })
        .done(function () {
            console.log('NETDATA.options.debug.main_Loop', NETDATA.options.debug.main_loop); // eslint-disable-line no-console
            if (NETDATA.options.debug.main_loop) {
